...
test[0] = await Menuitems("Wingacc")
await HomePage.logoutuser();

await ReviewUserPage.Devpro(555);
test[1] = await Menuitems("Devpro");
await HomePage.logoutuser();

await ReviewUserPage.Devplus(555);
test[2] = await Menuitems("Devplus");
await HomePage.logoutuser();

await ReviewUserPage.Devbasis(555);
test[3] = await Menuitems("Devbasis");
await HomePage.logoutuser();

await ReviewUserPage.Devlight(555);
test[4] = await Menuitems("Devlight");
await HomePage.logoutuser();
...
// console.log(expected)
    ...
    else if (abonnement.match("hoofdgebruiker")){
        let checker = arr => arr.every(Boolean);
        await CRMsub();
        await Inkoopsub();
        await Verkoopsub();
